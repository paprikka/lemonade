...
const apiResponseToCommunityEntries = (apiResponse: FediAPICommunityEntry[]): CommunityEntry[] => {
	return apiResponse.map((entry) => ({
		id: entry.id,
		instanceDomain: entry.url,
		name: entry.community.name,
		title: entry.community.title,
		description: entry.community.description ? entry.community.description.slice(0, 400) : '',
